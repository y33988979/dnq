...
dnq_queue_t *dnq_queue_create(U32 queue_size)
{
    ...
    queue = dnq_os_malloc(malloc_size);
    if(queue == NULL)
    {
    ...
    queue->head = 0;
    queue->tail = 0;
    queue->msg = (dnq_msg_t*)((U32)queue + sizeof(dnq_queue_t));
...
dnq_queue_t *dnq_queue_create_ex(U32 element_size, U32 queue_size)
{
    ...
    queue->head = 0;
    queue->tail = 0;
    queue->msg = (dnq_msg_t*)((U32)queue + sizeof(dnq_msg_t));

    for(i=0; i<queue_size; i++)
    {
        /* static ptr inside the message buffer, Not allowed to be changed!! */
        queue->msg[i].data = (U8*)((U32)&queue->msg[i] + sizeof(dnq_msg_t));
...
S32 dnq_msg_send(dnq_queue_t *queue, dnq_msg_t *msg)
{
    ...
    {
        DNQ_ERROR(DNQ_MOD_OS, "invalid pointer! queue=0x%08x, msg=0x%08x\n", queue, msg);
        return -1;
    }
...
S32 dnq_msg_recv(dnq_queue_t *queue, dnq_msg_t *msg)
{
    ...
    if(sem_wait(&queue->sem) < 0)
    {
        if(errno != ETIMEDOUT)
        {
            DNQ_ERROR(DNQ_MOD_OS, "sem_wait error: %s", strerror(errno));
            dnq_msleep(300);
        }
        return -1;
    }
...
S32 dnq_msg_recv_timeout(dnq_queue_t *queue, dnq_msg_t *msg, U32 timeout)
{
    ...
    if((ret=sem_timedwait(&queue->sem, &ts)) != 0)
    {
        if(errno != ETIMEDOUT)
        {
            DNQ_ERROR(DNQ_MOD_OS, "sem_timedwait error: %s", strerror(errno));
            dnq_msleep(300);
        }
        return -1;
    }
...
dnq_task_t* dnq_task_create(U8 *name, U32 stack_size, void *func, void *param)
{
    ...
    task = (dnq_task_t *)dnq_os_malloc(sizeof(dnq_task_t));
    if(!task)
    {
    ...
    void *param)
{
    ...
    appinfo = (dnq_appinfo_t *)dnq_os_malloc(sizeof(dnq_appinfo_t));
    if(appinfo == NULL)
    {
        DNQ_ERROR(DNQ_MOD_OS, "name %s: dnq_os_malloc appinfo error!", name);
        return NULL;
    }
